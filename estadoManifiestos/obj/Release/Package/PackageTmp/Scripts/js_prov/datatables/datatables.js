 ...
 *   https://datatables.net/download/#bs4/dt-1.10.16/af-2.2.2/b-1.5.1/b-colvis-1.5.1/b-flash-1.5.1/b-html5-1.5.1/cr-1.4.1/sc-1.4.4/sl-1.2.5
 *
 * Included libraries:
 *   DataTables 1.10.16, AutoFill 2.2.2, Buttons 1.5.1, Column visibility 1.5.1, Flash export 1.5.1, HTML5 export 1.5.1, ColReorder 1.4.1, Scroller 1.4.4, Select 1.2.5
 */
...
(function (factory) {
	...
	(function ($, window, document, undefined) {
		...
		DataTable.ext = _ext = {
			 ...
			 *  @type string
			 */
			build: "bs4/dt-1.10.16/af-2.2.2/b-1.5.1/b-colvis-1.5.1/b-flash-1.5.1/b-html5-1.5.1/cr-1.4.1/sc-1.4.4/sl-1.2.5",
	...
	$.extend(AutoFill.prototype, {
		...
		_drawSelection: function (target, e) {
			...
			var end = {
				row: dt.rows({
					page: 'current'
				}).nodes().indexOf(endCell.parent()[0]),
			...
			for (var rowIdx = 0; rowIdx < rows.length; rowIdx++) {
				selected.push(
					$.map(columns, function (column) {
						var cell = dt.cell(':eq(' + rows[rowIdx] + ')', column + ':visible', {
							page: 'current'
						});
						var data = cell.data();
...
(function (factory) {
	...
	var _excelSpecials = [{
		...
		}, // Numbers with thousand separators
		{
			match: /^\-?[\d,]+\.\d{2}$/,
			style: 64
		} // Numbers with 2 d.p. and thousands separators
	];
...
(function (factory) {
	...
	$.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo, drop, invalidateRows) {
		var i, iLen, j, jLen, jen, iCols = oSettings.aoColumns.length,
			nTrs, oCol;
		...
		"_fnCreateDragNode": function () {
			...
			this.dom.pointer = $('<div></div>')
				...
				.css({
					position: 'absolute',
					top: scrolling ?
						$('div.dataTables_scroll', this.s.dt.nTableWrapper).offset().top : $(this.s.dt.nTable).offset().top,
					height: scrolling ?
						$('div.dataTables_scroll', this.s.dt.nTableWrapper).height() : $(this.s.dt.nTable).height()
				})
				.appendTo('body');
		},
